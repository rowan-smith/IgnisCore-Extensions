...
final class BlockScanSupport {
    ...
    static IgnisLocation findNearestMatching(IgnisWorld world, IgnisLocation center, int radius, Set<String> targets) {
    ...
    static IgnisLocation findNearestOre(IgnisWorld world, IgnisLocation center, int radius) {
        return findNearestMatching(world, center, radius, ORE_HINTS);
    }

    static int countCrops(IgnisWorld world, IgnisLocation center, int radius) {
        int count = 0;
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
                for (int y = -2; y <= 2; y++) {
    ...
    static void bonemealRadius(IgnisWorld world, IgnisLocation center, int radius) {
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
    ...
    static void mossifyNearWater(IgnisWorld world, IgnisLocation center, int radius) {
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
    ...
    static void deoxidizeCopper(IgnisWorld world, IgnisLocation center, int radius) {
        for (int x = -radius; x <= radius; x++) {
            for (int y = 0; y <= 2; y++) {
                for (int z = -radius; z <= radius; z++) {
