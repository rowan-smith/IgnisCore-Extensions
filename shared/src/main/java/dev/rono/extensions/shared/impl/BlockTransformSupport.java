 ...
 */
final class BlockTransformSupport {
    ...
    static void transformSphere(IgnisWorld world,
    ...
    static void acidCorrode(IgnisWorld world, IgnisLocation center, int radius) {
        transformSphere(world, center, radius, ACID_TARGETS::contains, "air");
    }

    static void frostTransform(IgnisWorld world, IgnisLocation center, int radius) {
        int r2 = radius * radius;
        for (int x = -radius; x <= radius; x++) {
            for (int y = -radius; y <= radius; y++) {
                for (int z = -radius; z <= radius; z++) {
    ...
    static void igniteFireRing(IgnisWorld world, IgnisLocation center, int radius) {
    ...
    static void plantWildfireSpiral(IgnisWorld world, IgnisLocation center, int arms, int length) {
    ...
    static void mudslideFlow(IgnisWorld world,
                                     IgnisLocation center,
                                     int radius,
                                     int depth,
    ...
    static void tsunamiWave(IgnisWorld world, IgnisLocation center, int radius, IgnisScheduler scheduler) {
        int[] ring = {1};
        IgnisTask[] ref = {null};
        ref[0] = scheduler.runRepeating(center, () -> {
            if (ring[0] > radius) {
    ...
    static float breachingPower(BlockDefinition definition, float basePower) {
        return basePower; // multiplier applied per-block in breaching charge behavior
    }

    static boolean isBreachingWeak(String material) {
        return BREACHING_WEAK.contains(normalize(material));
    }

    static boolean isBreachingStrong(String material) {
        String m = normalize(material);
        return m.contains("stone") || m.equals("deepslate") || m.equals("bedrock");
    }

    static void infernoPatch(IgnisWorld world, IgnisLocation center, int radius) {
        int r2 = radius * radius;
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
                if (x * x + z * z > r2) {
    ...
    static void spreadInferno(IgnisWorld world,
                                    IgnisLocation center,
                                    int radius,
                                    int durationTicks,
                                    IgnisScheduler scheduler) {
    ...
    static void poisonCloud(IgnisWorld world,
                                    IgnisLocation center,
                                    double radius,
                                    int durationTicks,
                                    IgnisScheduler scheduler) {
    ...
    static void blackHoleCollapse(IgnisRegionService region,
                                          IgnisWorld world,
                                          IgnisLocation center,
                                          int voidRadius,
    ...
    static void breachingBlast(IgnisWorld world, IgnisLocation center, int radius) {
        int r2 = radius * radius;
        for (int x = -radius; x <= radius; x++) {
            for (int y = -radius; y <= radius; y++) {
                for (int z = -radius; z <= radius; z++) {
    ...
    static int customInt(BlockDefinition definition, String key, int defaultValue) {
