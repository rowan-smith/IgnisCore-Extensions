 ...
 */
final class ConsumableSupport {
    private ConsumableSupport() {
    }

    static boolean isOnCooldown(IgnisNbtService nbt, IgnisItem item, String key, long cooldownTicks) {
        if (nbt == null) {
            return false;
        }
    ...
    static void markUsed(IgnisNbtService nbt, IgnisItem item, String key) {
        if (nbt == null) {
            return;
        }
    ...
    static void consumeOne(IgnisItem item) {
        item.setAmount(Math.max(0, item.getAmount() - 1));
    }

    static void notifyCooldown(IgnisPlayer player, long remainingMs) {
