...
final class OreSnifferOnBlockPlaceListener implements OnBlockPlaceListener {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        PlacedTickSupport.start(context, event.block().location(), period, () -> OreSnifferSupport.tick(context, event.block().definition(), event.block().location()));
        IgnisLocation center = Locations.toCenter(event.block().location());
        TheatricsSupport.chime(OreSnifferSupport.worldAt(context, center), center, 1.0f);
    }
}
