...
class BehaviorTest {
    ...
    void triggerProducesBlast() {
        ...
        BlockDefinition definition = ExtensionTestSupport.loadBlockDefinition(
                BehaviorTest.class, "bouncing-betty", 10001);
        Strategy strategy = TestEventBus.activate(() -> new Strategy(ctx.context()), "bouncing-betty");
        RuntimeBlockInstance instance = BehaviorTestSupport.blockInstance(definition);

        ctx.eventBus().fireBlockTrigger(new BlockTriggerEvent(instance, null), "bouncing-betty");
