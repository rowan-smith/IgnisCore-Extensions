...
final class KelpCompressorOnBlockPlaceListener implements OnBlockPlaceListener {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        PlacedTickSupport.start(context, event.block().location(), period, () -> KelpCompressorSupport.tick(context, event.block().definition(), event.block().location()));
        IgnisLocation center = Locations.toCenter(event.block().location());
        TheatricsSupport.chime(KelpCompressorSupport.worldAt(context, center), center, 1.0f);
    }
}
