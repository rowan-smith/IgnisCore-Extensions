...
final class HydroponicTrayOnBlockPlaceListener implements OnBlockPlaceListener {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        ExtensionShared.ticks().start(context, event.block().location(), period, () -> HydroponicTraySupport.tick(context, event.block().definition(), event.block().location()));
        IgnisLocation center = Locations.toCenter(event.block().location());
        ExtensionShared.theatrics().chime(HydroponicTraySupport.worldAt(context, center), center, 1.0f);
    }
}
