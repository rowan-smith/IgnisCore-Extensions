...
final class OreEchoOnBlockPlaceListener implements OnBlockPlaceListener {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        PlacedTickSupport.start(context, event.block().location(), period, () -> OreEchoSupport.tick(context, event.block().definition(), event.block().location()));
        IgnisLocation center = Locations.toCenter(event.block().location());
        TheatricsSupport.chime(OreEchoSupport.worldAt(context, center), center, 1.0f);
    }
}
