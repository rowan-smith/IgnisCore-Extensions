...
final class AutoSieveOnBlockPlaceListener implements OnBlockPlaceListener {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        ExtensionShared.ticks().start(context, block.location(), period, () -> AutoSieveSupport.tick(context, block));
        IgnisLocation center = Locations.toCenter(block.location());
        ExtensionShared.theatrics().chime(context.extensions().resolveWorld(center), center, 1.0f);
    }
}
